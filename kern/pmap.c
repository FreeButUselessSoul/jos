...
page_init(void)
{
	...
	// free pages!
	size_t i;
    //boot_alloc(0) gets the address of previously allocated page
    ...
    cprintf("pages:%08x,current page addr:%08x\n", pages, (uint32_t)boot_alloc(0));
    pages[0].pp_ref = 1;
    for (i = 1; i < npages_basemem; i++) {
		if (i == MPENTRY_PADDR/PGSIZE) {
			pages[i].pp_ref = 1; // in use
			continue;
		}
        pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
    ...
    cr = page_free_list;
    page_free_list = cr->pp_link;
    cr->pp_link = NULL;
    if (alloc_flags & ALLOC_ZERO)
        memset(page2kva(cr), 0, PGSIZE);
    return cr;
}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	uintptr_t temp = base;
	base += ROUNDUP(size, PGSIZE); // update static base
	return (void *)temp;
}
