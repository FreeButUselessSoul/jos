...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall) {
		uintptr_t addr = UXSTACKTOP - sizeof(struct UTrapframe);
		struct UTrapframe *utf;
		if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP) { // already in user stack
			addr = tf->tf_esp - sizeof(struct UTrapframe) - sizeof(uintptr_t); // empty 32-bit word
		}
